...
export async function awaitApprovalDecision(input: AwaitApprovalInput): Promise<ApprovalRequest> {
  ...
  while (Date.now() - started <= input.timeoutMs) {
    const request = await readApprovalRequest(input.storeDir, input.id);
    if (!request) {
      throw new Error(`Approval request not found: ${input.id}`);
    }

    const current = markExpiredIfNeeded(request);
    if (current.status !== "pending") {
      if (current.status === "expired") {
        await writeApprovalRequest(input.storeDir, current);
      }
      return current;
    }
